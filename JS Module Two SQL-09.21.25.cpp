...
#include <tuple>
#include <vector>
#include <regex>  // ADDED: Required for std::regex
...
bool run_query(sqlite3* db, const std::string& sql, std::vector< user_record >& records)
{
    ...
    // Regular expressions used to detect common SQL injection patterns.
    std::regex sql_injection_pattern(".*(or\\s+\\d+=\\d+|or\\s+\\'[^']*'='[^']*').*", std::regex::icase);

    // Check if input SQL matches the injection pattern.
    if (std::regex_search(sql, sql_injection_pattern)) {
        std::cout << "Potential SQL Injection attack detected. Query execution aborted." << std::endl;
        return false;
    }
